...
import { View, StyleSheet, useColorScheme, Text, Animated, PanResponder } from 'react-native';
...
const smoothHistory = (points: TimelinePoint[]): TimelinePoint[] => {
	...
	for (let i = 0; i < points.length; i += 1) {
		...
		const smoothedValue = weightTotal > 0 ? weightedSum / weightTotal : points[i].value;
		smoothed.push({
...
const buildTileDataFromApi = (tileId: string, rows: TileApiRow[]): TileMockData => {
	const now = new Date();
	...
	rows.forEach((row) => {
		...
		const minutesFromStart = (timestamp.getTime() - windowStart.getTime()) / 60000;
...
const LineChart: React.FC<LineChartProps> = ({ data, accentColor, isDark, labelTextStyle }) => {
	...
	return (
		<View>
			...
			>
				{layout.width > 0 && layout.height > 0 && points.map((point, index) => {
					...
					const cy = LINE_CHART_VERTICAL_PADDING + (1 - point.y) * innerHeight;

					const elements: React.ReactElement[] = [];
					if (index > 0) {
						const prev = points[index - 1];
						const prevCx = LINE_CHART_HORIZONTAL_PADDING + prev.x * innerWidth;
						const prevCy = LINE_CHART_VERTICAL_PADDING + (1 - prev.y) * innerHeight;
						const dx = cx - prevCx;
						const dy = cy - prevCy;
...
const TileDetailsCard: React.FC<TileDetailsCardProps & { isVisible: boolean; onClose: () => void }> = ({ tile, colorScheme, isVisible, onClose }) => {
	...
	const subtleAccent = isDark ? 'rgba(34, 197, 94, 0.14)' : 'rgba(34, 197, 94, 0.12)';
	...
	useEffect(() => {
		...
		const load = async () => {
			try {
				const response = await fetch(`${API_BASE_URL}/map/${tile.tileId}?duration=1`);
				if (!response.ok) {
	...
	if (!tile) {
		content = (
			<>
				<Text style={[styles.bottomCardTitle, textPrimaryStyle]}>Explore the crowd</Text>
				<Text style={[styles.bottomCardSubtitle, textMutedStyle]}>
					Tap any colored square on the map to see live crowd levels for that area.
				</Text>
			</>
		);
		...
		content = (
			<>
				<Text style={[styles.bottomCardTitle, textPrimaryStyle]}>Loading data…</Text>
			</>
		);
	} else if (!tileData && !isLoading) {
		content = (
			...
			<>
				<View style={styles.statsRow}>
					<View style={styles.statItem}>
						...
						<Text style={[styles.statValue, textPrimaryStyle]}>{data.currentCount}</Text>
					</View>
				</View>
...
export const HomeScreen = () => {
	...
	const handleTilePress = useCallback((tile: { tileId: string; row: number; col: number }) => {
